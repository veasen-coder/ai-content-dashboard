...
export default function IncomePage() {
  ...
  const fetchData = useCallback(async () => {
    setRefreshing(true);
    try {
      const res = await fetch(`/api/supabase/finance?month=${month}&type=income`);
      if (res.ok) {
  ...
  return (
    <PageWrapper title="Income" lastSynced={lastFetched}>
      <div className="space-y-6">
        ...
        <div className="grid grid-cols-1 gap-4 sm:grid-cols-3">
          ...
          <div className="rounded-xl border border-[#1E1E1E] bg-[#111111] p-5">
            ...
            <p className="mt-2 text-2xl font-bold font-mono text-[#10B981]">
              {entries.length > 0 ? formatMYR(totalIncome / entries.length) : "RM 0.00"}
            </p>
          </div>
        </div>
        ...
        {loading ? (
          ...
          <div className="rounded-xl border border-[#1E1E1E] bg-[#111111]">
            <div className="flex items-center justify-between border-b border-[#1E1E1E] px-4 py-3">
              <h3 className="text-sm font-semibold">All Income — {getMonthLabel(month)}</h3>
            ...
            {entries.length > 0 ? (
              <div>
                {entries.map((entry) => (
                  ...
                  >
                    <div className="flex h-8 w-8 shrink-0 items-center justify-center rounded-lg bg-[#10B981]/20">
                      <ArrowUpRight className="h-4 w-4 text-[#10B981]" />
                    </div>
                    <div className="flex min-w-0 flex-1 flex-col">
                      <span className="truncate text-sm font-medium text-foreground">
                        {entry.description || entry.category || "Income"}
                      </span>
                    ...
                    <div className="text-right">
                      <p className="text-sm font-mono font-medium text-[#10B981]">
              ...
              <div className="flex flex-col items-center justify-center py-12">
                <p className="text-sm text-muted-foreground">
                  No income for {getMonthLabel(month)}
                </p>
              </div>
            )}
          </div>
